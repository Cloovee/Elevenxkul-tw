...
<div class="w-full max-w-lg">
    ...
    <div class="bg-white rounded-3xl p-7 shadow-[0_10px_30px_-18px_rgba(46,43,85,0.35)]">
        ...
        <h1 class="font-display text-xl font-semibold mb-1">Tambah Pelatih</h1>
        <p class="text-sm text-inksoft mb-6">Lengkapi biodata pelatih dan kaitkan ke salah satu ekskul yang kamu bina.</p>
        ...
        <form method="POST" action="{{ route('pembina.pelatih.store') }}" class="flex flex-col gap-4">
                ...
                <select name="id_ekskul" required
                    ...
                    @forelse($ekskuls as $e)
                        <option value="{{ $e->id_ekskul }}" {{ old('id_ekskul') == $e->id_ekskul ? 'selected' : '' }}>
                            {{ $e->nama_ekskul }}{{ $e->pelatih ? ' (akan menggantikan: '.$e->pelatih->nama_pelatih.')' : '' }}
                        </option>
                    @empty
            ...
            <div>
                ...
                <textarea name="alamat" rows="2"
                          class="w-full px-3.5 py-2.5 rounded-xl border border-[#DDE8FB] text-sm focus:outline-none focus:border-lavender">{{ old('alamat') }}</textarea>
            </div>
