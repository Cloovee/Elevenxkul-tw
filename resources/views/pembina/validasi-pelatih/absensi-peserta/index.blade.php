...
<body class="font-body antialiased bg-bgsoft text-ink">
    ...
    <div class="py-8 px-4 sm:px-6 lg:px-10 min-h-screen relative overflow-hidden">
        ...
        <div class="animate-fade-in-up rounded-3xl p-7 bg-[#D9F9DF] shadow-[0_10px_30px_-18px_rgba(31,122,61,0.35)] relative overflow-hidden">
            ...
            @else
                <div class="flex flex-col gap-3" x-data="{ openEkskul: 0 }">
                    @foreach ($riwayatPerEkskul as $i => $item)
                        ...
                        @endphp
                        <div class="border border-[#E8F0FE] rounded-2xl overflow-hidden">

                            {{-- TOMBOL KARTU EKSKUL: nama ekskul | hadir/total peserta | tanggal submit --}}
                            <button type="button"
                                    @click="openEkskul = (openEkskul === {{ $i }} ? null : {{ $i }})"
                                    class="w-full flex items-center justify-between gap-4 px-4 py-3.5 text-left bg-[#FFF9DB] hover:bg-[#FFF3B8] transition-colors">
                                <div class="flex items-center gap-3 min-w-0">
                                    <div class="w-10 h-10 rounded-full bg-gray-300 text-white text-xs font-bold flex items-center justify-center shrink-0">
                                        {{ $initial }}
                                    </div>
                                    <div class="min-w-0">
                            ...
                            <div class="sm:hidden flex items-center justify-between px-4 py-2 bg-[#FFF9DB] text-xs text-inksoft border-t border-white/60">
                            ...
                            <div x-show="openEkskul === {{ $i }}" x-transition class="bg-white" x-cloak>
                                @forelse ($item['sesi'] as $j => $sesiItem)
                                    <div x-data="{ openTanggal: false }" class="border-t border-[#F2F7FF]">
                                        <button type="button" @click="openTanggal = !openTanggal"
                                            ...
                                            <span class="text-sm font-medium flex items-center gap-2">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" class="text-[#1F7A3D] shrink-0"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                                                {{ $sesiItem['tanggal']->translatedFormat('d F Y') }}
                                            </span>
                                            <span class="flex items-center gap-3 shrink-0">
                                                <span class="text-[11px] font-bold px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700">{{ $sesiItem['hadir'] }}/{{ $sesiItem['total'] }} hadir</span>
                                            ...
                                            <table class="w-full text-sm border-collapse min-w-[520px]">
                                                <thead>
                                                    <tr class="text-inksoft text-[11px] uppercase tracking-wide">
                                                        ...
                                                        <th class="text-left py-2 px-2 border-b border-[#E8F0FE]">Kelas</th>
                                                        <th class="text-left py-2 px-2 border-b border-[#E8F0FE]">Status</th>
                                                        <th class="text-left py-2 px-2 border-b border-[#E8F0FE]">Aksi</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @foreach ($sesiItem['records'] as $a)
                                                        <tr class="border-b border-[#F2F7FF] last:border-none">
                                ...
                                @empty
                                    <div class="px-5 py-6 text-center border-t border-[#F2F7FF]">
