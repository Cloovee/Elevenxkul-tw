...
<div class="flex gap-5 p-5 min-h-screen">
    ...
    @include('pembina.partials.sidebar', ['active' => 'ketua'])
            ...
            <div class="flex items-center justify-between flex-wrap gap-4 relative">
                ...
                <div class="flex items-center gap-3">
                    <div class="bg-white/70 rounded-2xl px-4 py-2.5 text-center">
                        <div class="font-display text-xl font-bold text-[#3F41B0]">{{ $ketua->total() }}</div>
                        <div class="text-[11px] text-inksoft">Ketua terdaftar</div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-wrap justify-between items-center gap-4 mb-5">
                <form method="GET" class="flex flex-wrap items-center gap-2">
                    <div class="relative">
                        ...
                        <input type="text" name="search" placeholder="Cari nama/NISN/NIS ketua..." value="{{ request('search') }}"
                            class="pl-10 pr-4 py-2 bg-bgsoft border-none rounded-full text-sm text-ink placeholder-inksoft focus:ring-2 focus:ring-lavender w-64">
                    </div>
                    <button type="submit" class="px-4 py-2 bg-bgsoft hover:bg-[#e9edfb] text-ink rounded-full text-sm font-bold transition-colors">
                        Cari
                    </button>
                    @if(request('search'))
                        <a href="{{ route('pembina.ketua.index') }}" class="px-4 py-2 bg-bgsoft hover:bg-[#e9edfb] text-inksoft rounded-full text-sm font-bold transition-colors">
                            Reset
                        </a>
                    @endif
                </form>

                <a href="{{ route('pembina.ketua.create') }}"
                ...
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="text-left text-inksoft text-[11px] font-bold uppercase tracking-wider border-b border-gray-100">
                            ...
                            <th class="px-3 py-3">NISN / NIS</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($ketua as $key => $s)
                        @php $initial = strtoupper(substr($s->nama_siswa, 0, 1)); @endphp
                        <tr class="hover:bg-bgsoft/60 transition-colors">
                            <td class="px-3 py-3 text-inksoft font-medium">{{ $ketua->firstItem() + $key }}</td>
                            ...
                            <td class="px-3 py-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-ink font-bold whitespace-nowrap">{{ $s->nama_siswa }}</p>
                                        <p class="text-inksoft text-xs">{{ $s->email ?? '-' }}</p>
                                    </div>
                                </div>
                            </td>

                            <td class="px-3 py-3 text-inksoft whitespace-nowrap">
                                {{ $s->NISN }}<br><span class="text-xs">{{ $s->NIS }}</span>
                            </td>
                            ...
                            <td class="px-3 py-3 text-inksoft whitespace-nowrap">{{ $s->nama_kelas }}</td>

                            <td class="px-3 py-3 whitespace-nowrap">
                                @if($s->ekskulDipimpin)
                                    <span class="inline-block px-2 py-1 rounded-full text-[10px] font-bold bg-periwinkle/20 text-[#5E5CC7]">{{ $s->ekskulDipimpin->nama_ekskul }}</span>
                                @else
                                    <span class="text-inksoft">-</span>
                                @endif
                            </td>

                            <td class="px-3 py-3 text-inksoft whitespace-nowrap">{{ $s->user->username ?? '-' }}</td>
                            <td class="px-3 py-3 text-inksoft whitespace-nowrap">{{ $s->nomor_hp ?? '-' }}</td>

                            <td class="px-3 py-3">
                                <div class="flex gap-2">
                                    <a href="{{ route('pembina.ketua.edit', $s->id_siswa) }}" class="w-8 h-8 flex items-center justify-center rounded-lg bg-amber-50 text-amber-500 hover:bg-amber-500 hover:text-white transition-colors" title="Edit">
                                    ...
                                    <form action="{{ route('pembina.ketua.destroy', $s->id_siswa) }}" method="POST" onsubmit="return confirm('Yakin hapus ketua ini? Akun login-nya akan dihapus, tapi biodata siswa tetap tersimpan.')">
                                        @csrf @method('DELETE')
                        ...
                        <tr>
                            <td colspan="9" class="text-center py-12">
                                <div class="flex flex-col items-center text-inksoft">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="mb-2"><path d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                    <p class="font-bold text-sm">Belum ada Ketua terdaftar.</p>
                                    <p class="text-xs mt-1">Klik "Tambah Ketua" untuk mendaftarkan ketua ekskul kamu.</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
