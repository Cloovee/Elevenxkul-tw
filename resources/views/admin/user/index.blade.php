...
<div class="bg-white rounded-3xl shadow-[0_10px_30px_-18px_rgba(46,43,85,0.35)] border border-ink/5 p-6">
    ...
    <div class="flex flex-wrap justify-between items-center gap-4 mb-4">
        <h3 class="text-xl font-display font-bold text-[#2E2B55]">Kelola Akun User</h3>
        ...
        <form method="GET" class="flex flex-wrap items-center gap-2">
            ...
            <select name="role" class="px-4 py-2 bg-[#F3F4FC] border-none rounded-full text-sm text-[#2E2B55] focus:ring-2 focus:ring-[#9FA1FF]">
            ...
            <a href="{{ route('admin.user.index') }}" class="px-4 py-2 bg-[#F3F4FC] hover:bg-[#EEF0FD] text-[#6B6795] rounded-full text-sm font-bold transition-colors">
                Reset
            </a>
        </form>
    </div>
    ...
    <div class="flex flex-wrap gap-2 mb-6 pb-6 border-b border-ink/10">
        <a href="{{ route('admin.user.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-tr from-[#9FA1FF] to-[#7B77E0] text-white rounded-full text-sm font-bold shadow-md shadow-[#9FA1FF]/30 hover:opacity-90 transition-opacity">
        ...
        <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y divide-ink/5">
                ...
                <tr class="hover:bg-[#F3F4FC]/60 transition-colors">
                    <td class="px-3 py-3 text-[#6B6795] font-medium">{{ $users->firstItem() + $key }}</td>
                    ...
                    <td class="px-3 py-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-[#2E2B55] font-bold leading-tight">{{ $u->name }}</p>
                                <p class="text-[#6B6795] text-xs">{{ $u->email }}</p>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-3 py-3">
                        @if($u->role == 'Ketua')
                            @if($u->siswa)
                                <span class="text-[#2E2B55] font-semibold">{{ $u->siswa->nama_siswa }}</span>
                            @else
                        ...
                        @elseif($u->role == 'Pembina')
                            @if($u->pembina)
                                <span class="text-[#2E2B55] font-semibold">{{ $u->pembina->nama_pembina }}</span>
                            @else
                                <span class="inline-flex items-center gap-1 text-amber-600 text-xs font-bold bg-amber-50 px-2 py-1 rounded-full">
                            ...
                            @endif
                        @else
                            <span class="text-[#6B6795]">&mdash;</span>
                        @endif
                    </td>
