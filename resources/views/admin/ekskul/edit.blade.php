...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-[1.5rem] shadow-[0_4px_24px_-8px_rgba(0,0,0,0.05)] border border-gray-50 p-6 lg:p-8">
        ...
        <div class="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
            <div>
                <h3 class="text-xl font-extrabold text-[#2b3674]">Edit Ekskul</h3>
                <p class="text-xs font-medium text-[#a3aed1] mt-1">Ubah data ekstrakurikuler {{ $ekskul->nama_ekskul }}.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="id_ketua" id="id_ketua"
                        ...
                        @foreach($calonKetua as $s)
                            <option value="{{ $s->id_siswa }}" {{ old('id_ketua', $ekskul->id_ketua) == $s->id_siswa ? 'selected' : '' }}>
                                {{ $s->nama_siswa }}
                            </option>
                        @endforeach
                    </select>
                    <p class="mt-1.5 text-xs text-[#a3aed1]">
                        Cuma siswa yang sudah punya akun role Ketua yang muncul di sini.
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4"
                        class="w-full px-4 py-3 bg-[#f4f7fe] border @error('deskripsi') border-red-500 @else border-transparent @enderror rounded-xl text-sm text-[#2b3674] placeholder-[#a3aed1] focus:outline-none focus:bg-white focus:border-[#868dfb] focus:ring-2 focus:ring-[#868dfb]/20 transition-all">{{ old('deskripsi', $ekskul->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1.5 text-xs text-red-500 font-semibold flex items-center gap-1">
                            <i class="fas fa-exclamation-circle"></i> {{ $message }}
                        </p>
