...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-3xl shadow-[0_10px_30px_-18px_rgba(46,43,85,0.35)] border border-ink/5 p-6 lg:p-8">
        ...
        <div class="flex justify-between items-center mb-6 pb-4 border-b border-ink/10">
            <div>
                <h3 class="text-xl font-display font-bold text-[#2E2B55]">Tambah Ekskul Baru</h3>
                <p class="text-xs font-medium text-[#6B6795] mt-1">Lengkapi formulir di bawah untuk menambahkan ekstrakurikuler baru.</p>
            </div>
        ...
        <form action="{{ route('admin.ekskul.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="mt-1.5 text-xs text-[#6B6795]">Pembina yang dipilih akan bisa mengelola data pelatih untuk ekskul ini.</p>
                    @error('id_pembina')
                        <p class="mt-1.5 text-xs text-red-500 font-semibold flex items-center gap-1">
                            <i class="fas fa-exclamation-circle"></i> {{ $message }}
                        </p>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4"
                        ...
                        class="w-full px-4 py-3 bg-[#F3F4FC] border @error('deskripsi') border-red-500 @else border-transparent @enderror rounded-xl text-sm text-[#2E2B55] placeholder-[#6B6795] focus:outline-none focus:bg-white focus:border-[#9FA1FF] focus:ring-2 focus:ring-[#9FA1FF]/20 transition-all">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1.5 text-xs text-red-500 font-semibold flex items-center gap-1">
                            <i class="fas fa-exclamation-circle"></i> {{ $message }}
                        </p>
