...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-3xl shadow-[0_4px_24px_-8px_rgba(0,0,0,0.05)] border border-gray-50 p-6 lg:p-8">
        ...
        <div class="flex justify-between items-center mb-6 pb-4 border-b border-ink/10">
            <div>
                <h3 class="text-xl font-extrabold text-[#10316B]">Tambah Ekskul Baru</h3>
                <p class="text-xs font-medium text-[#7C8DB5] mt-1">Lengkapi formulir di bawah untuk menambahkan ekstrakurikuler baru.</p>
            </div>
        ...
        <form action="{{ route('admin.ekskul.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="mt-1.5 text-xs text-[#7C8DB5]">Pembina yang dipilih akan bisa mengelola data pelatih untuk ekskul ini.</p>
                    @error('id_pembina')
                        <p class="mt-1.5 text-xs text-red-500 font-semibold flex items-center gap-1">
                            <i class="fas fa-exclamation-circle"></i> {{ $message }}
                        </p>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4"
                        ...
                        class="w-full px-4 py-3 bg-[#F2F7FF] border @error('deskripsi') border-red-500 @else border-transparent @enderror rounded-xl text-sm text-[#10316B] placeholder-[#7C8DB5] focus:outline-none focus:bg-white focus:border-[#0B409C] focus:ring-2 focus:ring-[#0B409C]/20 transition-all">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1.5 text-xs text-red-500 font-semibold flex items-center gap-1">
                            <i class="fas fa-exclamation-circle"></i> {{ $message }}
                        </p>
