...
@section('content')
<div class="bg-white rounded-3xl shadow-[0_4px_24px_-8px_rgba(0,0,0,0.05)] border border-gray-50 p-8 max-w-2xl mx-auto">
    ...
    <form action="{{ route('admin.siswa.import') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-5">
            <label class="block text-xs font-bold text-[#7C8DB5] uppercase tracking-wider mb-2">Pilih File Excel</label>
            <input type="file" name="file" accept=".xlsx,.xls,.csv" required
                class="w-full px-4 py-2.5 bg-[#F2F7FF] border-none rounded-xl text-sm text-[#10316B] file:mr-4 file:py-1.5 file:px-4 file:rounded-full file:border-0 file:bg-[#0B409C] file:text-white file:text-xs file:font-bold hover:file:opacity-90">
            <p class="text-xs text-[#7C8DB5] mt-2">Format: .xlsx, .xls, .csv &middot; Maks 5MB</p>
        </div>

        <div class="mb-6">
            <label class="block text-xs font-bold text-[#7C8DB5] uppercase tracking-wider mb-2">Kelas Default (Opsional)</label>
